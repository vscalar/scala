  ...
  def reduce(st: State): State =
    ...
    (k, s, h, mem) match
      ...
      case (IDef(xs: List[String], env: Env, body: Expr) :: cont, s, h, mem) => 
        ...
        val newEnv = xs.zip(addrList).map((x, a) => (x -> a)).foldLeft(env)(_ + _)
        val newMem = addrList.zip(varList.reverse).map((a, v) => (a -> v)).foldLeft(mem)(_ + _)
      ...
      case (ICall(n)::cont, s, h, mem) =>
        ...
        stack match
          case CloV(params, body, fenv)::s => 
            val paramsNum = params.length
            val sBody = 
              if (n >= paramsNum) varList.drop(n - paramsNum)
              else List.fill(paramsNum - n)(UndefV) ++ varList

            val kvReturn = KValue(cont, s, h)
            val hBody = h ++ Seq(Return -> kvReturn) -- List(Break, Continue, Yield)
            State(IDef(params, fenv, EReturn(body))::Nil, sBody, hBody, mem)

          case GenV(params, body, fenv)::s => 
            val paramsNum = params.length
            val sBody = 
              if (n >= paramsNum) varList.drop(n - paramsNum)
              else List.fill(paramsNum - n)(UndefV) ++ varList

            val contBody = List(IPop, IDef(params, fenv, EReturn(ETry(body, "x", EId("x")))))
            val kvBody = ContV(KValue(contBody, sBody, Map()))
            val addr = malloc(mem)
            State(cont, IterV(addr)::s, h, mem + (addr -> kvBody))

          case _ => error()
      
      case (Nil, v::Nil, h,mem) => State(Nil,v::Nil,h,mem)

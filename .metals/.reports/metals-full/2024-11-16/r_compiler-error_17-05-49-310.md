  ...
  def reduce(st: State): State =
    ...
    (k, s, h, mem) match
      ...
      case (IDef(xs: List[String], env: Env, body: Expr)::cont, s, h,mem) => 
        if (s.length < xs.length) error() else ()
        val listLength = xs.length
        val valList = s.take(listLength)
        val addrList = malloc(mem, listLength)
        val newEnv = env ++ xs.zip(addrList).toMap
        val newMem = mem ++ addrList.zip(valList).toMap
        State(IEval(newEnv, body)::cont, s.drop(listLength), h, newMem)
      ...
      case (IYield::_, v1::b::v2::_, h, mem) => State(List(),List(NumV(0)),Map(),Map())

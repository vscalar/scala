  ...
  def reduce(st: State): State =
    ...
    (k, s, h, mem) match
      case (IEval(env: Env, expr: Expr) :: cont, s, h, mem) => expr match
        ...
        case EWhile(cond: Expr, body: Expr) => State(List(),List(NumV(0)),Map(),Map()) //State(IEval(env,cond)::IJmpIf(KValue(IEval(env, body)::IJmp(Continue)::Nil, s, h + ()))::cont,UndefV::s,h,mem)
        ...
        case EApp(fun: Expr, args: List[Expr]) =>
          println()
          println(List(IEval(env,fun)):::args.map(x => IEval(env, x))::ICall(args.length)::Nil)
          //if (args.length > 0) State(IEval(env,fun)::args.map(x => IEval(env, x))::ICall(args.length)::cont,s,h,mem)
          //else State(IEval(env,fun)::ICall(0)::cont,s,h,mem)
          State(List(),List(NumV(0)),Map(),Map())
